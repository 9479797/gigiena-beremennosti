...
**В маршруте книги «Гигиена беременности» (Editorial v1.8) отдельного этапа методической экспертизы нет: применимость оценивает врач-рецензент вместе с медицинской проверкой (решение пользователя 17.09.2026; роль акушерки-редактора упразднена)** (`editorial/human-review-roles-v1.0.md`, раздел 3; `editorial/pregnancy-book-editorial-v1.8.md`, раздел 8). Живой специалист с практикой проверяет ту же педагогическую логику плюс реальность применения. Модельный `methodical-review` в закреплённой точке маршрута больше не запускается и `USER GATE` в маршруте сам по себе не создаёт.
...
`явные решения пользователя → медицинская безопасность и заключение врача-рецензента → approved canon → content map → pregnancy-book-editorial-v1.8 → независимые проверки → исходный текст`.
...
Это маршрутизация для разового вызова. В самом маршруте книги (v1.7) применимость оценивает врач-рецензент вместе с медицинской проверкой, а не этот навык.
