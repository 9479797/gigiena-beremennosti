...
**В маршруте книги «Гигиена беременности» (Editorial v1.7) отдельного этапа методической экспертизы нет: применимость оценивает врач-рецензент вместе с медицинской проверкой (решение пользователя 17.09.2026; роль акушерки-редактора упразднена)** (`editorial/human-review-roles-v1.0.md`, раздел 3; `editorial/pregnancy-book-editorial-v1.7.md`, раздел 8). Живой специалист с практикой проверяет ту же педагогическую логику плюс реальность применения. Модельный `methodical-review` в закреплённой точке маршрута больше не запускается и `USER GATE` в маршруте сам по себе не создаёт.
